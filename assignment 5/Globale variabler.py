...
meter = int(input("Antall meter objektet skal ramle: "))

def get_fall_time(meter):
    tid = sqrt(((2*meter)/GRAVITY))
    return tid

print("Det tar", get_fall_time(meter), "sekunder å ramle", meter, "meter")
...
# Svar: Denne løsningen er mer brukervennlig og lar brukeren kontinuerlig endre på variabelen etter nødvendighet.
...
meter = int(input("Antall meter objektet skal ramle: "))
ny_gravity = float(input("Hvilken gravitasjonsakselerasjon skal brukes? "))

def get_fall_time(meter, ny_gravity):
    global GRAVITY
    GRAVITY = ny_gravity
    tid = sqrt(((2*meter)/GRAVITY))
    return tid

print("Det tar", get_fall_time(meter, ny_gravity), "sekunder å ramle", meter, "meter")
